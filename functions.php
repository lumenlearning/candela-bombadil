...
function bombadil_theme_scripts() {
	...
	wp_enqueue_script( 'iframe_resizer', get_stylesheet_directory_uri() . '/js/iframe_resizer.js', array( 'jquery' ), '', false );
	...
	wp_enqueue_script( 'hide_answers', get_stylesheet_directory_uri() . '/js/hide-answer.js', array( 'jquery' ), '', true );
...
function bombadil_color_picker_assets( $hook ) {
	...
	wp_enqueue_script( 'color-picker', get_stylesheet_directory_uri() . '/js/color-pick.js', array( 'wp-color-picker' ), false, true );
}
...
function ca_get_links( $echo = true ) {
	...
	if ( $echo ) { ?>
		<div class="bottom-nav-buttons">
			<?php if ( '/' != $prev_chapter ) { ?>
				<a class="page-nav-btn" id="prev" href="<?php echo esc_url( $prev_chapter ); ?>">
					<?php _e( 'Previous', 'pressbooks' ); ?>
				</a>
			<?php } ?>

			<?php if ( '/' != $next_chapter ) { ?>
				<a class="page-nav-btn" id="next" href="<?php echo esc_url( $next_chapter ); ?>">
...
function show_nav_container() {
	...
	if ( ( 1 == $navigation['nav_show_header'] ) || ( 1 == $navigation['nav_show_search'] ) ) {
		return true;
	}
}
